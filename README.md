...
In the ablation study (reconstruction, multiscale, roatation & noise invariance), the model is trained on 7 major categories from ShapeNet. The experiment is then conducted within our [ShapeNet](https://drive.google.com/open?id=1Pmu9e70uKBvxgBYbjU8GxuzLY3xWPc0x)  evaluation set with 9 extra held-out categories. The evaluation for classification uses the official train/test split. The classifier used in the final evaluation of computed descriptors is an MLP.
